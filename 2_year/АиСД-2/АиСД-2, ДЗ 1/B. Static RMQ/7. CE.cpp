...
//int64_t
long long int sum_of_segment(int v, int left, int right, int l, int r) {
    if (r <= left || right <= l) {  // 1
        return 0;
    }
    if (l <= left && right <= r) {  // 2
    ...
    int mid = (left + right) / 2;
    int64_t first_child = sum_of_segment(2 * v + 1, left, mid, l, r);
    int64_t second_child = sum_of_segment(2 * v + 2, mid, right, l, r);
...
int main() {
    ...
    for (int j = 0; j < Q; ++j) {
        ...
        cin >> left_j >> right_j;
        if (T_i == 1) {
            // Сумма по границам включительна поэтому right + 1
            cout << sum_of_segment(0, 0, n, left_j, right_j + 1) << '\n';
        }
        if (T_i == 2) {
